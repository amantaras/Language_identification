...
class TranscriptionSegment:
    def __init__(
        self, seg_id: int, language: str, start_sec: float, end_sec: float, text: str
    ):
        self.id = seg_id
...
def _transcribe_chunk(
    samples, sample_rate: int, host: str, language: str, key: str, billing: str
) -> str:
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        if billing:
            speech_config.set_property(
                speechsdk.PropertyId.SpeechServiceConnection_Endpoint, billing
            )
        if key:
            speech_config.set_property(
                speechsdk.PropertyId.SpeechServiceConnection_Key, key
            )
...
def transcribe_segments(
    audio_file: str,
    segments_json: str,
    language_host_map: Dict[str, str],
    key: str,
    billing: str,
    out_path: str,
):
    ...
    for seg in segments:
        ...
        text = _transcribe_chunk(
            chunk, sample_rate=sr, host=host, language=lang, key=key, billing=billing
        )
        out_segments.append(
            TranscriptionSegment(
                seg["id"], lang, seg["start_sec"], seg["end_sec"], text
            )
        )

    with open(out_path, "w", encoding="utf-8") as f:
        json.dump(
            {"audio": audio_file, "segments": [s.to_dict() for s in out_segments]},
            f,
            ensure_ascii=False,
            indent=2,
        )
